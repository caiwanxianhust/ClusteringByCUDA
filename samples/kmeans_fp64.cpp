...
int readCoordinate(const char *file, DataType *data, int *label, const int n_features, int N) {
    std::ifstream ifs;
...
void timing(
    ...
    clustering::KmeansGPU<DataType> *model = new clustering::KmeansGPU<DataType>(clusters, num_clusters, n_features, n_samples);
...
void launchDemo(int max_num_samples) {
    ...
    DataType *data = new DataType[data_buf_size];
    int *label = new int[data_buf_size];

    const char *file = "./sample_1e5_fea_500_class_8_lable_1_ninfo_8.csv";
    int num_samples = readCoordinate<DataType>(file, data, label, n_features, max_num_samples);
    ...
    DataType clusters[cluster_buf_size] = {0};
    ...
    timing<DataType>(data, label, clusters, num_clusters, n_features, num_samples);
